...
const readCodingRules = async () => {
    ...
    if (codingGuide.path != null) {
        ...
        return chunked
            ...
            .map((chunk) => {
            const [, filePattern = "^$"] = chunk.match(codingGuide.filePattern) ?? [];
            return { rule: chunk, filePattern: RegExp(filePattern, "m") };
        });
    }
    throw new Error("Please set CODING_GUIDE_PATH and CODING_GUIDE_LEVEL, or CODING_GUIDE_READER.");
};
const main = async () => {
    ...
    if (git.commentId && git.comment) {
        ...
        if (!git.comment.startsWith("/ai-review-flex")) {
            console.log("Skip AI review because the comment doesn't starts with /ai-review-flex.");
            return;
        }
        ...
        for (const df of splitForEachDiff(diff)) {
            ...
            for (const rule of rules) {
                ...
                promises.push(async () => {
                    ...
                    const reviewComments = await review(rule.rule, path, df.diff, env.language);
                    for (const comment of reviewComments) {
                        if (env.debug) {
                            console.debug(`Receive REVIEW (${randomId})`, comment);
                        }
                        git.postReviewComment(path, comment.start, comment.end, comment.body);
                        commentedCount += 1;
                    }
                });
            }
