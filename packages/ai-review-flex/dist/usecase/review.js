...
const buildComment = (str, codingRule) => {
    let start = 0;
    let end = 0;
    ...
    if (start === 0 || end === 0 || body === "")
        return undefined;
    body = appendCodingRule(body, codingRule);
    return { start, end, body };
};
const parseResponse = (response, codingRule) => {
    const comments = [];
    let buf = "";
    const lines = response.match(/.*?((#+)?\s*Review Comment[\s\S]*)/m)?.[0].split("\n") ??
        [];
...
// Main
// ----------------------------------------------------------------------
export const review = async (codingRule, filePath, diff, language) => {
